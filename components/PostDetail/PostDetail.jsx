...
import { getContentFragment } from './postDetails.utils';
        ...
        <div className='px-4 lg:px-0'>
          ...
          {post.content.raw.children.map((typeObj, index) => {
            const children = typeObj.children.map((item, itemIdx) => {
              // for bullet list items or numbered list
              if (item.type === 'list-item') {
                return item.children.map((list) => {
                  if (list.type === 'list-item-child') {
                    const itemList = list.children.map((itemList) => itemList);
                    const listItemChild = itemList.map(
